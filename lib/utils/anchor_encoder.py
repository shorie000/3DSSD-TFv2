...
def encode_angle2class_tf(angle, num_class):
    ...
    angle = tf.math.floormod(angle, (2 * np.pi))

    angle_per_class = 2*np.pi/float(num_class)

    # shifted_angle = tf.mod(angle+angle_per_class/2, 2*np.pi)
    shifted_angle = tf.math.floormod(angle+angle_per_class/2, 2*np.pi)
...
def encode_bin_residual(assigned_res, half_bin_search_range, bin_num_class):
    ...
    bin_cls = tf.floor(
        (assigned_res + half_bin_search_range) / bin_interval_range)
...
def encode_log_anchor(gt_ctr, gt_offset, anchor_ctr, anchor_offset):
    ...
    anchor_l, anchor_h, anchor_w = tf.unstack(
        anchor_offset, axis=-1)  # [bs, points_num]
...
def encode_log_anchor_np(gt_ctr, gt_offset, anchor_ctr, anchor_offset):
    ...
    anchor_l, anchor_h, anchor_w = np.split(
        anchor_offset, 3, axis=-1)  # [bs, points_num, 1]

    anchor_d = np.concatenate([anchor_l, anchor_w],
                              axis=-1)  # [bs, points_num, 2]
    anchor_d = np.linalg.norm(
        anchor_d, axis=-1, keepdims=True)  # [bs, points_num, 1]

    gt_x, gt_y, gt_z = np.split(gt_ctr, 3, axis=-1)  # [bs, points_num, 1]
    anchor_x, anchor_y, anchor_z = np.split(
        anchor_ctr, 3, axis=-1)  # [bs, points_num, 1]
...
def encode_dist_anchor_free(gt_ctr, gt_offset, anchor_ctr, anchor_offset=None):
    ...
    padding_translate = tf.stack(
        [padding_zeros, padding_half_height, padding_zeros], axis=-1)  # [bs, points_num, 3]
...
def encode_bin_anchor(gt_ctr, gt_offset, anchor_ctr, anchor_offset, half_bin_search_range, bin_num_class):
    ...
    x_bin, x_res = encode_bin_residual(
        gt_x - anchor_x, half_bin_search_range, bin_num_class)
    z_bin, z_res = encode_bin_residual(
        gt_z - anchor_z, half_bin_search_range, bin_num_class)
