...
class TargetAssigner:
    ...
    def assign(self, points, anchors_3d, gt_boxes_3d, gt_labels, gt_angle_cls, gt_angle_res, gt_velocity, gt_attribute, valid_mask=None):
        ...
        assigned_idx, assigned_pmask, assigned_nmask = self.assign_targets_anchors(
            points, anchors_3d, gt_boxes_3d, gt_labels, valid_mask)  # [bs, points_num, cls_num]

        assigned_gt_labels = self.gather_class(
            gt_labels, assigned_idx)  # [bs, points_num, cls_num]
        assigned_gt_labels = assigned_gt_labels * \
            tf.cast(assigned_pmask, tf.int32)
        ...
        if gt_attribute is not None:
            # bs, npoint, cls_num
            assigned_gt_attribute = self.gather_class(
                gt_attribute, assigned_idx)
        else:
            assigned_gt_attribute = None

        returned_list = [assigned_idx, assigned_pmask, assigned_nmask, assigned_gt_boxes_3d, assigned_gt_labels,
                         assigned_gt_angle_cls, assigned_gt_angle_res, assigned_gt_velocity, assigned_gt_attribute]
    ...
    def gather_class(self, gt_labels, assigned_idx):
        ...
        assigned_gt_labels = tf.squeeze(
            tf.cast(assigned_gt_labels, gt_labels_dtype), axis=-1)
    ...
    def iou_assign_targets_anchors(self, points, anchors_3d, gt_boxes_3d, gt_labels, valid_mask):
        ...
        if self.iou_sample_type == 'BEV':
            ...
            iou_matrix = query_points_iou(
                points, anchors_3d_reshape, gt_boxes_3d, iou_3d)
        ...
        assigned_idx, assigned_pmask, assigned_nmask = tf.py_func(gt_sampler.iou_assign_targets_anchors_np,
                                                                  [iou_matrix, points, anchors_3d, gt_boxes_3d, gt_labels, self.minibatch_size,
                                                                   self.positive_ratio, self.pos_iou, self.neg_iou, self.effective_sample_range, valid_mask],
    ...
    def mask_assign_targets_anchors(self, points, anchors_3d, gt_boxes_3d, gt_labels, valid_mask):
        ...
        assigned_idx, assigned_pmask, assigned_nmask = tf.compat.v1.py_func(gt_sampler.mask_assign_targets_anchors_np,
                                                                            [points, anchors_3d, gt_boxes_3d, gt_labels, self.minibatch_size, self.positive_ratio,
                                                                             self.pos_iou, self.neg_iou, self.effective_sample_range, valid_mask],
                                                                            [tf.int32, tf.float32, tf.float32])
