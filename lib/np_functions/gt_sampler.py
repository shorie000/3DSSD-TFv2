...
def iou_guided_nms(iou_matrix, pred_boxes_3d, pred_scores, pred_iou_3d, iou_thresh):
    ...
    Args:
        ...
        iou_thresh: float
    """
    keep_idx, pred_boxes_3d, pred_scores = cython_nms.matrix_iou_guided_nms(
        iou_matrix, pred_boxes_3d, pred_scores, pred_iou_3d, iou_thresh)
...
def iou_assign_targets_anchors_np(batch_iou_matrix, batch_points, batch_anchors_3d, batch_gt_boxes_3d, batch_gt_labels, minibatch_size, positive_rate, pos_iou, neg_iou, effective_sample_range, valid_mask):
    ...
    for i in range(bs):
        ...
        # [pts_num, cls_num]
        dist = np.linalg.norm(
            cur_anchors_3d[:, :, :3] - assigned_gt_boxes[:, np.newaxis, :3], axis=-1)

        # then we get assigned_idx by whether a point is within an object
        filtered_assigned_idx = filter_idx[sampled_gt_idx]  # [pts_num]
        filtered_assigned_idx = np.tile(np.reshape(
            filtered_assigned_idx, [pts_num, 1]), [1, cls_num])
        batch_assigned_idx[i] = filtered_assigned_idx

        # then we generate pos/neg mask
        assigned_idx = np.tile(np.reshape(
            sampled_gt_idx, [pts_num, 1, 1]), [1, cls_num, 1])
        iou_mask = np.tile(np.reshape(np.arange(len(filter_idx)), [1, 1, len(filter_idx)]), [
                           pts_num, cls_num, 1])  # [pts_num, cls_num, len(filter_idx)]
        iou_matrix = np.sum(np.equal(iou_mask, assigned_idx).astype(
            np.float32) * iou_matrix, axis=-1)  # [pts_num, cls_num]
        if cls_num > 1:
            label_mask = np.tile(np.reshape(
                np.arange(cls_num), [1, cls_num]), [pts_num, 1])
            label_mask = np.equal(
                label_mask, assigned_gt_label[:, np.newaxis]).astype(np.float32)
        else:
            label_mask = np.ones([pts_num, cls_num], dtype=np.float32)
        iou_matrix = iou_matrix * label_mask + \
            (1-label_mask) * np.ones_like(iou_matrix) * -1  # count and ignored
        ...
        nmask = np.logical_and(np.less(iou_matrix, neg_iou), np.greater_equal(
            iou_matrix, 0.05)).astype(np.float32)
        ...
        if minibatch_size != -1:
            ...
            if cur_positive_num > 0:
                positive_inds = np.random.choice(
                    positive_inds, cur_positive_num, replace=False)
            ...
            if cur_negative_num > 0:
                negative_inds = np.random.choice(
                    negative_inds, cur_negative_num, replace=False)
...
def mask_assign_targets_anchors_np(batch_points, batch_anchors_3d, batch_gt_boxes_3d, batch_gt_labels, minibatch_size, positive_rate, pos_iou, neg_iou, effective_sample_range, valid_mask):
    ...
    for i in range(bs):
        ...
        pmask = np.logical_and(
            pmask[:, np.newaxis], dist_mask).astype(np.float32)
