    ...
    def build_layer(self, xyz_list, feature_list, bn_decay, output_dict):
        ...
        with tf.variable_scope(self.scope) as sc:
            for i, channel in enumerate(self.mlp_list):
                feature_input = self.conv_op(feature_input, channel, bn=self.bn, scope='conv1d_%d' % (
                    i), bn_decay=bn_decay, is_training=self.is_training)
            ...
            self.head_predictor(feature_input=feature_input,
